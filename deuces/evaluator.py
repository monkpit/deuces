...
class Evaluator(object):
    ...
    def hand_summary(self, board, hands):
        ...
        for i in range(len(stages)):
            line = ("=" * line_length) + " {stages} " + ("=" * line_length)
            print(line.format(stages=stages[i]))
            ...
            for player, hand in enumerate(hands):
                ...
                print("Player {player} hand = {class_string}, percentage rank among all hands = {percentage}".format(
                    player=player + 1, class_string=class_string, percentage=percentage))
            ...
            if i != stages.index("RIVER"):
                if len(winners) == 1:
                    print("Player {player} hand is currently winning.\n".format(player=winners[0] + 1))
                else:
                    print("Players {players} are tied for the lead.\n".format(players=' & '.join([x + 1 for x in winners])))
            ...
            # otherwise on all other streets
                ...
                if len(winners) == 1:
                    print("Player {player} is the winner with a {hand}\n".format(
                                player=winners[0] + 1,
                                hand=self.class_to_string(
                                        self.get_rank_class(
                                            self.evaluate(hands[winners[0]], board)))))
                else:
                    print("Players {players} tied for the win with a {hand}\n".format(
                            players=' & '.join(winners),
                            hand=self.class_to_string(self.get_rank_class(self.evaluate(hands[winners[0]], board)))))
